...
文件_ https://github.com/hooke007/MPV_lazy/discussions/186

極簡主義設計驅動的多功能界面腳本群組，相容 thumbfast 新縮圖引擎
]]
...
--bind_command('subtitles', create_select_tracklist_type_menu_opener({ -- 舊版次字幕菜單
...
mp.add_key_binding(nil, 'shot', function()
	if Menu:is_open() then
		...
		if paused then
			mp.add_timeout(timeout, function() -- 延遲過低可能產生閃爍
...
if options.idlescreen then
	mp.register_script_message('osc-idlescreen', function(mode, no_osd)
		...
		if not no_osd and mp.get_property_number('osd-level', 1) >= 1 then
			mp.osd_message('LOGO的可見性：' .. tostring(mode))
		end
	end)
end
